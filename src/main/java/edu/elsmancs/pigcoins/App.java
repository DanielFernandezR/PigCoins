...
public class App {
	...
	public static void main(String[] args) {
		/**
		 * Crea una wallet Genera las claves privada y publica de la wallet
		 */
		...
		/**
		 * Crea una segunda wallet, esta vez generando sus claves con un metodo
		 * wallet.generateKeyPair() que encapsula el codigo de la anterior historia de
		 * usuario
		 */
		...
		/**
		 * Crea una transaccion de pigcoins Visualiza la transaccion
		 */
		...
		/**
		 * Crea el blockchain y añade transacciones que crean moneda "pigcoins"
		 */
		...
		/**
		 * Indicar en la wallet el total de pigcoins que se han enviado, que se han
		 * recibido y el balance
		 */
		...
		/**
		 * Carga en la wallet el total de transacciones recibidas (aquellas que
		 * significan recibir pigcoins) y enviadas (aquellas que envian pigcoins) y
		 * mostrarlas
		 */
		...
		/**
		 * Recargamos la wallet_1 y visualizamos el blockchain
		 */
		...
		/**
		 * Enviar pigcoins de la wallet_1 a la wallet_2 Este es el flujo de trabajo que
		 * has de programar.
		 * 
		 * wallet_1.sendCoins(wallet_2.getAddress(), pigcoins, message, bChain) {
		 * 
		 * collectCoins(pigcoins); signTransaction(message);
		 * bChain.processTransactions(pKey_sender, pKey_recipient, consumedCoins,
		 ...
		 * isConsumedCoinValid(consumedCoins); createTransaction(pKey_sender,
		 * pKey_recipient, consumedCoins,message, signedTransaction); }
		 * 
		...
		/**
		 * Primero has de recolectar los suficientes pigcoins de tu wallet Para ello has
		 * de eliminar de las transacciones entrantes de la wallet las que ya se han
		 * utilizado para enviar pigcoins
		 */

		/**
		 * Los pigcoins son indivisibles, asi que si necesitas 5.2 y no tienes ninguna
		 * transaccion entrante exacta de 5.2, has de realizar un CHANGE ADDRESS (CA):
		 * Consiste en reutilizar una transaccion entrante mayor que 5.2, por ejemplo
		 * 10, y enviar dos transacciones: - una transaccion de 5.2 al destinatario -
		 * otra transaccion de 10 - 5.2 = 4.8 a ti mismo/a Ten cuidado: no puedes
		 * utilizar transacciones entrantes (pigcoins) que ya hayas enviado con
		 * anterioridad o el blockchain rechazara las nuevas
		 */
		...
		/**
		 * Una vez que recolectes los pigcoins (transacciones) de tu wallet envialas al
		 * blockchain. Debes firmar el mensaje con tu clave privada.
		 */

		String message = "he roto la hucha :(";
		byte[] signedTransaction = wallet_1.signTransaction(message); // usa GenSig.sign()
		wallet_1.sendCoins(wallet_2.getAddress(), pigcoins, message, bChain); // usa wallet.collectCoins() y
																				// bChain.processTransactions()

		/**
		 * wallet.sendCoins() invoca a bChain.processTransactions(wallet_1.getAddress(),
		 * wallet_2.getAddress(), consumedCoins, message, signedTransaction);
		 */

		/**
		 * El blockchain debe chequear que las transacciones entrantes no proceden de
		 * transacciones que ya se han utilizado (gastado), mediante el metodo:
		 * 
		 * boolean bChain.isConsumedCoinValid(consumedCoins);
		 */

		/**
		 * El blockchain debe chequear que las transacciones sean autenticas, es decir,
		 * que la firma del mensaje sea autentica, mediante el metodo:
		 * 
		...
		/**
		 * Si el blockchain comprueba que los pigcoins que envias satisfacen las dos
		 * condiciones anteriores,incluye estas transacciones en la cadena de bloques
		 * 
		 * bChain.processTransactions(wallet_1.getAddress(), wallet_2.getAddress(),
		 * consumedCoins, message, signedTransaction);
		 * bChain.createTransaction(pKey_sender, pKey_recipient, consumedCoins,message,
		 * signedTransaction);
		 */
